...
License:
--------
Gleb Averochkin, Ivan Zlobin, Ivan Bespalov, Eugeniy Alexandrov, Machine Learning with Physically Grounded, Interpretable Descriptors for Polymer Property Prediction and Monomer Design,
...
def calculate_volumes(original_mol):
    try:
        ...
        # mark '*' atoms and temporarily replace them with H to enable 3D embedding/optimization
        ...
        # --- 3D embedding: ETKDGv3 with fixed seed and increased attempts ---
        ...
        # --- Geometry optimization: only if a 3D conformer exists ---
        if result == 0 and mol.GetNumConformers() > 0:
            AllChem.UFFOptimizeMolecule(mol)

        # --- Distance between polymerization sites ('*' atoms) ---
        ...
        try:
            if mol.GetNumConformers() > 0:
                ...
                wildcard_indices = [a.GetIdx() for a in mol.GetAtoms() if a.HasProp("is_wildcard")]
        ...
        # Fallback: if no 3D, compute a 2D distance silently
        if distance_between_polymerization_sites is None:
            try:
                ...
                wildcard_indices = [a.GetIdx() for a in mol2D.GetAtoms() if a.HasProp("is_wildcard")]
                if len(wildcard_indices) == 2:
                    i1, i2 = wildcard_indices
        ...
        for i, atom in enumerate(mol.GetAtoms()):
            ...
            # base atomic volume (sphere) with pairwise overlap corrections
            ...
            for bond in atom.GetBonds():
                ...
                if atom_coords is not None:
                    ...
                    d = 1.5 * (r1 + r2)  # heuristic distance without coordinates
            ...
            atom_data = (atom.GetSymbol(), volume)

            # assign atom contribution to the corresponding bucket
            if i in main_VdW:
        ...
        print("💥 calculate_volumes failed:", repr(e))
        raise
...
def run_xtb(smiles: str, max_attempts: int = 3, base_seed: int = 42) -> dict:
    ...
    for attempt in range(max_attempts):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return desc
    print(f"[xTB FAIL] All {max_attempts} attempts failed for SMILES: {smiles}")
    return {}
...
def featurize_to_dict(SMILES, with_xtb: bool = True):
    """
    Function to compute all features with their full names for a SMILES string. 
    Returns a dictionary of feature : value.
